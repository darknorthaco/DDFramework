// FIPS 180-4 SHA-256 — pure Rust, zero dependencies, no unsafe.
//
// Shared between build.rs (via include!) and the main binary, so
// the hash computed at build time is bit-identical to the hash
// computed at runtime. Tested against NIST test vectors in
// tests/sha256_vectors.rs.
//
// This implementation is not optimized for speed. It is optimized
// for clarity, auditability, and long-term rebuildability.
//
// Note: this file is both a module (via lib.rs `pub mod sha256;`)
// AND is textually `include!`d by build.rs. It therefore cannot use
// inner attributes like `#![allow(...)]`.

#[allow(dead_code)]
const _SHA256_MODULE_MARKER: () = ();
